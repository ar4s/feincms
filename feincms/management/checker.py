...
def check_database_schema(cls, module_name):
    ...
    def _fn(sender, **kwargs):
        ...
        print(style.ERROR(
            'The following columns seem to be missing in the database'
            ' table %s:' % cls._meta.db_table))
        ...
        print(style.NOTICE(
            ...
            ' find out what the correct column types are. (Or use south, which'
            ' is what you should be doing anyway.)\n' % (
            cls._meta.app_label,
            )))
    return _fn
